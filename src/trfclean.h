...
IL *GetList(const char *datafile)
{
    ...
    while (1) {
        ...
        i = fscanf(fp, "%s %d %d %d %f %d %d %d %d %d %d %d %d %f %s",
            ...
            &newptr->acount, &newptr->ccount, &newptr->gcount, &newptr->tcount, &newptr->entropy, patbuffer);
...
int IsRedundant(IL *iptr, IL *jptr)
{
    ...
    if ((iptr->period > jptr->period) && (iptr->period % jptr->period == 0) && (iptr->score <= 1.1 * jptr->score))
        return 1;
    ...
    for (i = 1; i <= nfiles; i++) {
        ...
        for (j = 0; j < EO_MAX_TBL && currptr != NULL; currptr = currptr->next, j++) {
            ...
            fprintf(fp,
                ...
                linkfile, currptr->ref, currptr->first, currptr->last, currptr->period, currptr->copies, currptr->size,
                currptr->matches, currptr->indels, currptr->score, currptr->acount, currptr->ccount, currptr->gcount,
                currptr->tcount, currptr->entropy);
        }
